...
class StringField(PrimitiveField):
    """Unicode Strings.  ``str`` is used to wrap and unwrap values,
    and any subclass of str is an acceptable input"""
...
class RegExStringField(PrimitiveField):
    ...
    def __init__(self, regex, **kwargs):
        ...
        """
        self.regex = regex
        super(RegExStringField, self).__init__(constructor=str, **kwargs)
...
class NumberField(PrimitiveField):
    ...
    def schema_json(self):
        ...
        return dict(min_value=self.min, max_value=self.max, **super_schema)
...
class DateTimeField(PrimitiveField):
    ...
    def schema_json(self):
        ...
        return dict(
            min_date=self.min, max_date=self.max, use_tz=self.use_tz, **super_schema
        )
    ...
    def validate_wrap(self, value):
        ...
        if self.use_tz and value.tzinfo is None:
            self._fail_validation(
                value,
                """datetime is not timezone aware and use_tz is on.  make sure timezone is set on the session""",
            )
...
class ComputedField(Field):
    ...
    def schema_json(self):
        ...
        return dict(
            computed_type=self.computed_type.schema_json(),
    ...
    def __set__(self, instance, value):
        ...
        if obj_value.set and self.one_time:
            raise BadValueException(
                self._name, value, "Cannot set a one-time field once it has been set"
            )
    ...
    def dirty_ops(self, instance):
        ...
        return {self.on_update: {self._name: self.wrap(value)}}
    ...
    def compute_value(self, doc):
        ...
        try:
            ...
            self._fail_validation(
                value, "Computed Function return a bad value", cause=bve
            )
        return value
